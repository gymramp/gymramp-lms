...
import { getAllCompanies as fetchAllAccessibleBrands, getAllLocations as fetchAllSystemLocations, getLocationsByCompanyId, getCompanyById } from '@/lib/company-data';
...
// Timestamp is not directly used here, date strings from Firestore are parsed
...
export default function AdminUsersPage() {
  const [users, setUsers] = useState<EmployeeWithOverallProgress[]>([]); // Raw list based on brand filter
  const [filteredUsers, setFilteredUsers] = useState<EmployeeWithOverallProgress[]>([]); // Further filtered by location
  
  const [isLoading, setIsLoading] = useState(true); // General loading for user list
  const [isLoadingFilters, setIsLoadingFilters] = useState(true); // Loading for filter dropdowns
  ...
  const [allLocationsForSystem, setAllLocationsForSystem] = useState<Location[]>([]); // All locations user can see
  const [locationsForLocationFilter, setLocationsForLocationFilter] = useState<Location[]>([]); // Locations for the selected brand

  const [selectedBrandIdForFilter, setSelectedBrandIdForFilter] = useState<string>(''); // Can be 'all'
  const [selectedLocationIdForFilter, setSelectedLocationIdForFilter] = useState<string>('all'); // Can be 'all'
  const [lastGeneratedPasswordForNewUser, setLastGeneratedPasswordForNewUser] = useState<string | null>(null);
  
  const router = useRouter();
  const { toast } = useToast();

  // Effect for initial auth check and fetching data for filters
  useEffect(() => {
      ...
      if (firebaseUser && firebaseUser.email) {
        try {
          ...
          const [fetchedAccessibleBrands, fetchedAllSystemLocations] = await Promise.all([
            fetchAllAccessibleBrands(userDetails), // Fetches brands based on user role
            fetchAllSystemLocations(), // Fetches all locations (will be filtered client-side for managers)
          ]);
          setAccessibleBrandsForFilter(fetchedAccessibleBrands);
          
          if (userDetails.role === 'Manager' && userDetails.assignedLocationIds) {
             setAllLocationsForSystem(fetchedAllSystemLocations.filter(loc => userDetails.assignedLocationIds!.includes(loc.id)));
          } else {
             setAllLocationsForSystem(fetchedAllSystemLocations);
          }
          ...
          if (userDetails.role === 'Super Admin') {
            ...
            setSelectedBrandIdForFilter(userDetails.companyId);
             ...
             setSelectedBrandIdForFilter(''); // Should not happen for Admin/Owner/Manager if properly set up
          }
        } catch (error) {
  ...
  // Effect to populate locations for the location filter based on selected brand
  useEffect(() => {
    if (isLoadingFilters || !currentUser) {
      setLocationsForLocationFilter([]); setSelectedLocationIdForFilter('all'); return;
    }
    
    let currentBrandLocations: Location[] = [];
    if (selectedBrandIdForFilter === 'all') { // "All Accessible Brands"
      if (currentUser.role === 'Super Admin') {
        currentBrandLocations = allLocationsForSystem;
      } else { // Admin/Owner seeing all their accessible brands
    ...
    // For Managers, ensure location filter only shows THEIR assigned locations if "All Locations" is conceptually their scope
  ...
  // Effect to fetch users based on current filters
    ...
    try {
        ...
        if (currentUser.role === 'Super Admin') {
            ...
            if (selectedBrandIdForFilter === 'all') { // All *accessible* brands for Admin/Owner
                ...
                usersData = (await Promise.all(usersPromises)).flat().filter((user, index, self) => index === self.findIndex((u) => u.id === user.id)); // Deduplicate
            } else { // Specific brand selected by Admin/Owner (must be one they can access)
                if (accessibleBrandsForFilter.some(b => b.id === selectedBrandIdForFilter)) {
                    usersData = await getUsersByCompanyId(selectedBrandIdForFilter);
                } else { usersData = []; } // Should not happen if filter is populated correctly
            }
        } else if (currentUser.role === 'Manager' && currentUser.companyId) {
            // Managers always see users from their own brand. Location filtering happens next.
  ...
  }, [fetchUsersForCurrentFilters]); // Runs when filters or currentUser changes

  // Effect to apply location filtering to the fetched users
  useEffect(() => {
    if (isLoading || !currentUser) { setFilteredUsers([]); return; }
    
    let tempUsers = [...users]; // Start with users already filtered by brand
    ...
    if (selectedLocationIdForFilter && selectedLocationIdForFilter !== 'all') {
        ...
        // Manager's "All Locations" means all locations they are assigned to.
        // Filter employees to show only those in locations the manager manages.
        const managerLocations = currentUser.assignedLocationIds || [];
        if (managerLocations.length > 0) {
            tempUsers = tempUsers.filter(emp => 
                emp.id === currentUser.id || // Always include the manager
                (emp.assignedLocationIds || []).some(locId => managerLocations.includes(locId))
            );
        } else { // If manager has no assigned locations, they only see themselves
            tempUsers = tempUsers.filter(emp => emp.id === currentUser.id);
        }
    }
    // For Super Admin and Admin/Owner, if "All Locations" is selected, tempUsers remains as is (all users for selected brand(s)).
  ...
  const refreshUsersAndShowPassword = (user: User, tempPassword?: string) => {
     fetchUsersForCurrentFilters(); // Re-fetch users based on current filters
     if (tempPassword) {
  ...
  const handleAddUserClick = () => {
     ...
     // If Super Admin and 'All Brands' is selected, they might need to select a brand in the dialog
  ...
  const handleDeleteUser = async (userId: string, userName: string) => {
    const success = await deleteUser(userId); // This is a soft delete
    if (success) {
        fetchUsersForCurrentFilters(); // Re-fetch users
  ...
  const canPerformAction = (targetUser: User): boolean => {
    ...
    if (currentUser.role === 'Super Admin') return true; // Super Admin can do almost anything
    if (currentUser.id === targetUser.id) return false; // Cannot perform on self (except edit own profile elsewhere)
    
    const currentUserBrandId = currentUser.companyId;
    const targetUserBrandId = targetUser.companyId;

    if (!currentUserBrandId || !targetUserBrandId) return false; // Both must belong to a brand for non-SA

    // Check if target user's brand is within current user's accessible scope
  ...
  const handleResetFilters = () => {
      ...
      setSelectedLocationIdForFilter('all'); // This will trigger user fetch via useEffect on selectedBrandIdForFilter
  };

  const getInitials = (name?: string | null): string => name ? name.split(' ').map(n => n[0]).join('').toUpperCase() : '??';

  if (isLoadingFilters && !currentUser) { // Initial loading before user is even known
  ...
  if (!currentUser || !['Super Admin', 'Admin', 'Owner', 'Manager'].includes(currentUser.role)) {
     // This case should ideally be caught by the auth redirect in the first useEffect.
       ...
       <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-primary">User Management</h1>
         <div className="flex items-center gap-2">
             {/* AddUserDialog needs access to allLocationsForSystem for its internal brand-based location filtering */}
            <AddUserDialog 
                onUserAdded={refreshUsersAndShowPassword} 
                isOpen={isAddUserDialogOpen} 
                setIsOpen={setIsAddUserDialogOpen}
                companies={accessibleBrandsForFilter} // Pass accessible brands for the dropdown
                locations={allLocationsForSystem}   // Pass all relevant locations for the dialog's internal filtering
                currentUser={currentUser} 
            />
            <Button onClick={handleAddUserClick} className="bg-accent text-accent-foreground hover:bg-accent/90"
                disabled={isLoadingFilters || !currentUser || !['Super Admin', 'Admin', 'Owner', 'Manager'].includes(currentUser.role) || (accessibleBrandsForFilter.length === 0 && currentUser.role === 'Super Admin')}
                title={ (accessibleBrandsForFilter.length === 0 && currentUser?.role === 'Super Admin') ? "Add a brand first" : (!['Super Admin', 'Admin', 'Owner', 'Manager'].includes(currentUser?.role || 'Staff') ? "Permission Denied" : "")} >
         ...
         <h2 className="text-lg font-semibold mr-4 self-center">Filters:</h2>
           <div className="flex flex-col space-y-1">
             ...
             <Select value={selectedBrandIdForFilter}
               ...
               disabled={isLoadingFilters || (accessibleBrandsForFilter.length <= 1 && currentUser.role !== 'Super Admin')}
              >
               <SelectTrigger id="brand-filter" className="w-[220px] bg-background h-10">
                 <SelectValue placeholder="Select Brand" />
               </SelectTrigger>
               <SelectContent>
                 {/* Allow "All Accessible Brands" only if user can see more than one, or is Super Admin */}
                 ...
                 {/* Handle case where Admin/Owner only has their primary brand */}
                 {accessibleBrandsForFilter.length === 1 && (currentUser.role === 'Admin' || currentUser.role === 'Owner') &&
                 ...
                 <Select value={selectedLocationIdForFilter} onValueChange={(value) => setSelectedLocationIdForFilter(value)}
                    ...
                    <SelectContent>
                        ...
                        {locationsForLocationFilter.map(location => ( <SelectItem key={location.id} value={location.id}>{location.name}</SelectItem> ))}
                         {/* Show message if a specific brand is selected but has no locations */}
      ...
      <Card>
            ...
            <Table>
              ...
              <TableBody>
                {filteredUsers.map((user) => {
                    const brandForUser = accessibleBrandsForFilter.find(c => c.id === user.companyId) || allLocationsForSystem.find(l => l.companyId === user.companyId); // Fallback if brand not in accessible list due to complex filter scenarios
                    const brandName = brandForUser?.name || (user.companyId ? "Brand (Filtered Out)" : "N/A"); // Handle if brand name can't be found for user
                    const brandIsChild = !!(brandForUser && 'parentBrandId' in brandForUser && brandForUser.parentBrandId);
                    ...
                    return (
                        <TableRow key={user.id} className={cn(!user.isActive && "opacity-60")}>
                            ...
                            <TableCell> <span className="flex items-center gap-1 text-sm text-muted-foreground"> <Building className="h-4 w-4 opacity-70" /> {brandName} {brandIsChild && <Badge variant="outline" className="text-xs">Child</Badge>} {!user.companyId && user.role !== 'Super Admin' && <AlertTriangle className="h-4 w-4 text-yellow-500" title="Brand Missing"/>} </span> </TableCell>
                                    ...
                                    {user.id !== currentUser.id && (
                                      <>
                                        ...
                                        >
                                          ...
                                          <span>Deactivate User</span> {/* Changed from Delete to Deactivate */}
                                        </DropdownMenuItem>
                                      </>
                                    )}
                                  </DropdownMenuContent>
       ...
       {isEditUserDialogOpen && userToEdit && currentUser && ( 
         <EditUserDialog 
            ...
            currentUser={currentUser} 
            companies={accessibleBrandsForFilter} // Pass accessible brands
            locations={allLocationsForSystem}   // Pass all relevant locations
         /> 
        )}
    </div>
  );
